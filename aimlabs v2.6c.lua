...
Title.Position = UDim2.new(0.2200000, 0, 0, 0)
Title.Size = UDim2.new(0, 170, 0, 26)
Title.Font = Enum.Font.RobotoMono
Title.Text = "Aimlabs v2.6c"
...
coroutine.wrap(BWFAW_fake_script)() -- PASTED DRAG!!!!!! ZSKID!!!! (stfu your dad left you)
local function HYJWBV_fake_script() -- ESPToggle.LocalScript 
	...
	script.Parent.MouseButton1Click:Connect(function()
		...
		if script.Parent.Value.Value == false then
			TweenService:Create(script.Parent, TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(77,77,77)}):Play() -- i fucking hate tweenservice why dont you cast dictionary some bitches
		elseif script.Parent.Value.Value == true then
		    TweenService:Create(script.Parent, TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(255, 255, 255)}):Play()
...
local function DZPSUBJ_fake_script() -- TeamToggle.LocalScript 
	...
	script.Parent.MouseButton1Click:Connect(function()
		...
		if script.Parent.Value.Value == false then
			TweenService:Create(script.Parent, TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(77,77,77)}):Play()
		elseif script.Parent.Value.Value == true then
			TweenService:Create(script.Parent, TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(255, 255, 255)}):Play()
		end
	end)
...
local function YPIWDK_fake_script() -- Ping.LocalScript 
	...
	function getStat(t)
		...
		elseif sta == 'sent' then
			return sthandler.NetworkSent:GetValueString():split(".")[1] -- PASTED !!!! FROM DEVFORUMS! !!!! SKID!!! (no shit im not making my own ping counter when 500 people have already done it better)
		end
	end
...
local function UAUZV_fake_script() -- FPS.LocalScript 
	...
	while wait() do
		script.Parent.Text = string.sub(FPS, 1, 8) -- PASTED !!!! FROM DEVFORUMS! !!!! SKID!!! (no shit im not making my own fps counter when 500 people have already done it better)
	end
	
end
coroutine.wrap(UAUZV_fake_script)()

local ESP = loadstring(game:HttpGet("https://raw.githubusercontent.com/centerepic/turbo-octo-garbanzo/main/KIRIOT%20ESP%20AIMLABS%20MODIFICATION.lua"))()
...
if aimlabsUniversalAimbotLoaded == true then
	Notify("aimlabs", "Script Loaded Already", "", 3) -- probably the best aimbot if it didnt fucking break for no reason
	return 
end
ESP:Toggle(ESPEnabled)
ESP.Names = true
...
task.wait(5)
SeparateNotify("aimlabs", "Only R6 is currently supported. (Will R15 add soon™)", 5)