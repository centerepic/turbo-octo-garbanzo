--[[
	...
	car teleportation, sashashop (item selling bot), a working node copy (lolo), nodekill, item sniper (not too sure if athena has an item sniper tbh). just to name a few
...
local sashashopButton = Instance.new("TextButton")
...
sashashopButton.Name = "sashashopButton"
sashashopButton.Parent = miscScrollingFrame
sashashopButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
sashashopButton.Position = UDim2.new(0.222529814, 0, 0.540439129, 0)
sashashopButton.Size = UDim2.new(0, 119, 0, 34)
sashashopButton.Style = Enum.ButtonStyle.RobloxRoundDefaultButton
sashashopButton.Font = Enum.Font.Ubuntu
sashashopButton.Text = "[OFF] sashashop"
sashashopButton.TextColor3 = Color3.fromRGB(0, 0, 0)
sashashopButton.TextSize = 16.000
	...
	if _G.AlreadyLoaded then
		...
		tpToBaseplateButton.Activated:Connect(function()
			if _G.deathBarrierBypassEnabled then
			    Baseplatezz = Instance.new("Part",Workspace);
			    print('instance made');
			    Baseplatezz.Anchored = true;
			    Baseplatezz.Transparency = 0.2;
			    Baseplatezz.Size = Vector3.new(62, 1, 56);
			    Baseplatezz.Position = Vector3.new(-208.222, 10.1391, -395.508);
			    print('baseplate finisshd');
				game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-206, 16.8, -394);
				notice:Fire("Lunahax", "Teleported to baseplate");
				notice:Fire("sasher", "(ur welcome for the fix)");
			else
					...
					for i,v in pairs(allOpenTrades) do
						...
						pcall(function()
							if difference == v[2] then
								Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Hey " .. v[1] .. ", I recieved your payment, please stand still!"), "All");
								...
								if hasFoundTrade == false then
									...
									pcall(function()
										...
										if itemNamee == "" then
											Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("[/ad sashashop] Hey " .. v[1] .. ", Coulden't find the item, refunding..."), "All");
											...
											wasSuccess = true;
											isSellingItemRn = false;
											Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Hey " .. v[1] .. ", Teleported and dropped item to you!"), "All");
											controls:Enable();
											removeInProgTrade(tonumber(v[2]));
											table.remove(allOpenTrades, tablefind(allOpenTrades, v));
											wait(1);
						...
						for iaa,vaa in pairs(game.Workspace[game.Players.LocalPlayer.Name]:GetChildren()) do
							if string.lower(tostring(vaa.Name)) == buyersItem then
								shouldBreak = true;
								Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Hey " .. v[1] .. ", something went wrong, refunding you..."), "All");
								table.remove(allOpenTrades, tablefind(allOpenTrades, v));
								Game:GetService("ReplicatedStorage").Events.MenuActionEvent:FireServer(2, tostring(v[2]), tostring(buyersUsername));
								local controls = require(game:GetService("Players").LocalPlayer.PlayerScripts.PlayerModule):GetControls();
								controls:Enable();
					...
					for i,v in pairs(allOpenTrades) do
						pcall(function()
							...
							if v[3] < currTime then
								Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Hey " .. v[1] .. ", your trade expired!"), "All");
								table.remove(allOpenTrades, tablefind(allOpenTrades, v));
							end
						end)
					end
		...
		local function onPlayerChatted(player, message)
			if _G.sashashopEnabled then
				pcall(function()
					command = string.split(string.split(message, sashashopPrefix)[2], " ")[1];
						...
						spawn(function()
							...
							if isUserEnemy(username) then
								...
								if doCooldown(username) then
									if command == "buy" then
										if sashashopEnabled then
											if username == game.Players.LocalPlayer.Name then
												Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("[sashashop] You can't buy your own item!", "All");
											else
												itemToBuy = "";
														...
														if itemInStock(itemToBuy) then
															if getItemPrice(itemToBuy) > 0 then
																...
																if checkIfUsedPrice(uniqPrice) then
																	Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Hey " .. removeNumbers(username) .. ", Something went wrong!"), "All");
																else
																	addTrade(username, tonumber(uniqPrice), itemToBuy);
																	Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Hey " .. removeNumbers(username) .. ", Please send me EXACTLY $" .. uniqPrice .. "pesos!"), "All");
																	wait(1);
																	Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] The price is slightly higher so the bot knows who's who.", "All");
																end
															else
																Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] That item has been disabled for purchasing.", "All");
															end
														else
															Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Sorry, but that item isn't in stock, did you spell it correctly?", "All");
														end
													else
														Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Hey " .. removeNumbers(username) .. ", " .. getKeywordResponse(message)), "All");
													end
												end
											end
										else
											Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Sorry, Wish bot is disabled!", "All");
										end
									elseif command == "setprice" then
										if username == game.Players.LocalPlayer.Name then
											...
											pcall(function()
												...
												Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] " .. itemToSet .. " is now $" .. itemPrice .. "!"), "All");
												wasSuccess = true;
											end)
											if wasSuccess == false then
												Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Something went wrong, did you enter a item name + price?", "All");
											end
										end
									elseif command == "stock" then
										Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Current available stock (this might take a while):", "All");
										wait(2);
										for i,v in pairs(_G.prices) do
											pcall(function()
												...
												if itemInStock(itemToBuy) and getItemPrice(itemToBuy) > 0 then
													Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Item: " .. displayedItemName .. " Price: " .. getItemPrice(itemToBuy)), "All");
													wait(3);
												end
											end)
										end
										Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] End of list.", "All");
										...
										if itemToBuy ~= nil then
											...
											if getKeywordResponse(message) == "" then
												if itemInStock(itemToBuy) and getItemPrice(string.lower(itemToBuy)) > 0 then
													Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Item: " .. renamedItemName .. " Price: " .. getItemPrice(string.lower(itemToBuy))), "All");
												else
													Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Sorry, that item isn't available right now.", "All");
												end
											else
												Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] Hey " .. removeNumbers(username) .. ", " .. getKeywordResponse(message)), "All");
											end
									...
									elseif command == "disable" then
										if username == game.Players.LocalPlayer.Name then
											sashashopEnabled = false;
											Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Wish bot disabled!", "All");
										end
									elseif command == "enable" then
										if username == game.Players.LocalPlayer.Name then
											sashashopEnabled = true;
											Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Wish bot enabled!", "All");
										end
									elseif command == "help" then
										Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] " .. sashashopPrefix .. "help, " .. sashashopPrefix .. "buy (item) " .. sashashopPrefix .. "price (item), " .. sashashopPrefix .. "stock", "All");
										wait(1);
										Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] If you're trying to buy an item with a space", "All");
										wait(1);
										Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Please put quotes around it, example: \"Bloxy Cola\"", "All");
										wait(1);
										if username == game.Players.LocalPlayer.Name then
											Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop owner] " .. sashashopPrefix .. "enable, " .. sashashopPrefix .. "disable, " .. sashashopPrefix .. "setprice (item) (price), " .. sashashopPrefix .. "rename, " .. sashashopPrefix .. "prefix, " .. sashashopPrefix .. "bypass", "All");
										end
											...
											pcall(function()
												...
												if counterer > 1 then
													...
													if itemToSet ~= "" and itemRenamedName ~= "" then
														...
														Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bypassChat("/ad [sashashop] " .. itemToSet .. " is now " .. itemRenamedName .. "!"), "All");
														wasSuccess = true;
													end
												end
											end)
											if wasSuccess == false then
												Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Something went wrong, did you enter the item name + renamed name?", "All");
											end
										end
										...
										if username == game.Players.LocalPlayer.Name then
											...
											pcall(function()
												...
												if newPrfix ~= nil and envIsAlphaNum(newPrfix) == false then
													sashashopPrefix = newPrfix;
													Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] The prefix is now '" .. sashashopPrefix .. "'!", "All");
													wasSuccess = true;
												end
											end)
											if wasSuccess == false then
												Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Something went wrong, did you enter a prefix?", "All");
											end
										end
										...
										if username == game.Players.LocalPlayer.Name then
											...
											pcall(function()
												...
												if value == "true" then
													...
													wasSuccess = true;
													Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Chat bypass enabled!", "All");
												elseif value == "false" then
													bypassChatEnabled = false;
													wasSuccess = true;
													Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Chat bypass disabled!", "All");
												end
											end)
											if wasSuccess == false then
												Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] Something went wrong, did you enter true/false?", "All");
											end
										end
									end
								end
		...
		print("sashashop loaded");
		
		-- button
		sashashopButton.Activated:Connect(function()
			if _G.sashashopEnabled then
				_G.sashashopEnabled = false;
				sashashopButton.Text = "[OFF] sashashop"
				notice:Fire("Lunahax", "sashashop disabled.");
				Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] sashashop has been disabled.", "All");
			else
				_G.sashashopEnabled = true;
				sashashopButton.Text = "[ON] sashashop"
				notice:Fire("Lunahax", "sashashop enabled.");
				Game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/ad [sashashop] sashashop has been enabled! Type '/.ad !help' for info", "All");
			end
		end)
					...
					pcall(function()
						...
						if ownedPrinters >= 2 or printersMade >= 2 then
							...
							pcall(function()
                                ...
                                [1] = 1,
                                [2] = "Node",
                                [3] = CFrame.new(Vector3.new(-200.60067749023, 8.8069725036621, -424.32534790039), Vector3.new(-0.86602538824081, -3.7855173218304e-08, 0.5))
                                }
                                game:GetService("ReplicatedStorage").Events.BuildingEvent:FireServer(unpack(args))
								wait(1);
								game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-1169, 136, 394, -0.18111819, 0, -0.983460665, 0, 1, 0, 0.983460665, 0, -0.18111819);
								wait(1);
								...
								pcall(function()
									if tonumber(printer.Int2.Money.Value) < 99999 then
										game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-204.05764770508, 13.694646835327, -396.65289306641, 0, 0, 1, 0, 1, -0, -1, 0, 0);
										notice:Fire("sashahax","wow i fixed dis!!")
										didSuccessyy = true;
									end
								end)
		...
		addButton("sashashop", mainMenuScrollingFrame, function() 
			dabutton = game.Players.LocalPlayer.PlayerGui.ScreenGui.mainMenuFrame.miscFrame.miscScrollingFrame.sashashopButton;
			events = {"MouseButton1Click", "MouseButton1Down", "Activated"}
			for iasda,vaaaa in pairs(events) do
				for iasdaa,vaaa in pairs(getconnections(dabutton[vaaaa])) do
					vaaa:Fire()
